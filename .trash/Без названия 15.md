Вы правы, приношу извинения! В одном из сгенерированных оглавлений была допущена опечатка (явносные вместо явные), которая ломала ссылку.
Я также уточнил генерацию ссылок-якорей (часть [...](#...)), чтобы они лучше соответствовали GFM (GitHub Flavored Markdown), самому популярному стандарту. Например, якоря для 5.1. теперь будут вида #51-..., а не #51-....
Вот полная, исправленная версия документации. Скопируйте ее целиком.
...
2.  [Почему "явные" (Explicit) JOIN?](#2-почему-явные-explicit-join)
...
> Примечание о работоспособности ссылок:
> Разные программы для просмотра Markdown (VS Code, Obsidian, Typora, GitHub и т.д.) могут по-р