...
## ПРОМПТ 3: СОЗДАНИЕ ИНСПЕКТОРА

**Триггер:** После ПРОМПТ 2  
**Зависимости:** CREATE_CORE_SYSTEM  
**Следующие промпты:** любые разработческие

```
[ACTION] CREATE_INSPECTOR  
[PARENT_ACTION] CREATE_CORE_SYSTEM
[MODULE_ID] MODULE_Inspector_VER_1.0
[DEPENDENCIES] MODULE_SystemRegistry_VER_1.0

Создай систему отладки:
1. Inspector класс с методами activate(), highlight(), showInfo()
2. CSS стили для подсветки элементов
3. UI панель с информацией об элементах
4. Интеграция с SystemRegistry
5. Data-атрибуты для всех существующих модулей

ОБЯЗАТЕЛЬНЫЕ ОБНОВЛЕНИЯ:
- project_registry.json: добавить MODULE_Inspector_VER_1.0
- dependencies_map.json: связь Inspector → SystemRegistry  
- todo.json: закрыть TASK_002
- changelog.md: "Added Inspector for debugging"

СЛЕДУЮЩИЕ ДЕЙСТВИЯ:
1. ПРОМПТ 4: CREATE_MODULE (с отладкой)
2. ПРОМПТ 5: ADD_FEATURE (с инспектором)
```

---

## ПРОМПТ 4: СОЗДАНИЕ МОДУЛЯ

**Триггер:** Нужен новый модуль  
**Зависимости:** CREATE_CORE_SYSTEM  
...
Создай модуль:
1. Файл src/modules/{name}/index.js с уникальными ID
2. Data-атрибуты для Inspector
3. Регистрация в SystemRegistry
4. Документация README.md
5. Базовые тесты

ОБЯЗАТЕЛЬНЫЕ ОБНОВЛЕНИЯ:
- project_registry.json: новый модуль с ID и зависимостями
- dependencies_map.json: узел и связи
- todo.json: TASK для тестирования модуля
- changelog.md: "Added module {name}"
...
**Триггер:** После создания/изменения кода  
**Зависимости:** проверяемый модуль  
**Следующие промпты:** ПРОМПТ 8, ПРОМПТ 9

```
[ACTION] CODE_REVIEW
[PARENT_ACTION] {действие создавшее код}
[COMPONENT_ID] {ID компонента}
[CODE_TO_REVIEW] {вставь код для ревью}

ЗАГРУЗИ КОНТЕКСТ:
- Модуль из project_registry.json
- Зависимости из dependencies_map.json
- Тесты (если есть)
...
- Правильно ли используются data-атрибуты?
...
ОБЯЗАТЕЛЬНЫЕ ОБНОВЛЕНИЯ:
- todo.json: TASK для исправлений (если нужно)
- changelog.md: "Code review completed"

СЛЕДУЮЩИЕ ДЕЙСТВИЯ:
1. ПРОМПТ 8: FIX_CODE (если есть проблемы)
2. ПРОМПТ 9: REFACTOR (если нужны улучшения)
3. APPROVE (если все хорошо)
```
...
## ДЕТАЛЬНЫЙ ШАБЛОН INSPECTOR СИСТЕМЫ

### МОДУЛЬ: MODULE_System_Inspector_VER_1.0

#### ЦЕЛЬ
Интерактивный инспектор элементов для отладки и анализа архитектуры приложения.

#### ФУНКЦИОНАЛЬНОСТЬ

**1. Режим инспекции**
- Активация по Ctrl+Shift+I
- Подсветка элементов при наведении
- Панель информации об элементе

**2. Анализ элементов**
- Показ ID, типа, модуля
...
- Получение данных из SystemRegistry
- Отслеживание использования
- Генерация отчетов

#### ТЕХНИЧЕСКИЕ ДЕТАЛИ
- CSS-подсветка элементов
- Fixed overlay панель
- Keyboard navigation
- Performance monitoring

#### ОЖИДАЕМЫЙ ВЫВОД
Полный код инспектора включая:
- Класс Inspector
- HTML/CSS для панели
- Интеграционные хуки
- Примеры использования
